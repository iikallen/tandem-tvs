...
| ------------------------ | -------------------------------------------------------------------------------------------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Portal/news/search       | 180 constant VUs, part of 30-minute run                                                                  | Error rate <1%; feed/detail/search p95 <2 s  | `PASS` — 0%; 581/576/1,060ms                                                                       |
| Messenger HTTP           | 90 constant VUs, part of 30-minute run                                                                   | History/inbox p95 <2 s; no lost commit       | `PASS` — history 749ms; post-run state PASS                                                        |
| Realtime-active users    | 30 constant VUs, part of 30-minute run                                                                   | Healthy delivery p95 <1 s                    | `PASS` — message delivery 982ms; failures 0%                                                       |
| Authenticated WebSockets | 300 sockets held at least 15 minutes                                                                     | No crash/leak/DB exhaustion; reconnect works | `PASS` — observed 303; completed/full-duration 300/300; failures 0%                                |
...
The release mixed run observed a peak of 23 PostgreSQL connections across 385 five-second samples;
the dedicated WSS run peaked at 6. This leaves 377 connections of measured headroom against the
configured maximum. PostgreSQL reported three superuser-reserved and zero additional reserved
connections; rollback and deadlock counters remained zero in both runs.
...
Django production uses `CONN_MAX_AGE=0`, six Uvicorn workers and Celery concurrency 2. On the clean deterministic 300-user profile, six workers reduced realtime message delivery p95 from 1.59 s with four workers to 909.49 ms. Eight workers measured 910.20 ms, so six remained the smallest configuration worth accepting. The final default run measured 982ms and passed without connection exhaustion or process restart. The load harness chooses an eligible conversation deterministically by UUID/VU instead of repeatedly concentrating traffic in whichever conversation a previous run made hottest.
...
| ---------------------- | ------------------------------------------------------------------------------------- | ------------ |
| Feed/detail            | 1.308 / 0.401 ms                                                                      | None         |
| Global search sections | publications 5.992; comments 3.273; messages 86.713; files 0.818; employees 24.550 ms | None         |
| Inbox/history/unread   | 0.299 / 1.397 / 0.169 ms                                                              | None         |
| Notifications          | 0.226 ms                                                                              | None         |
| Analytics              | 184.554 ms                                                                            | None         |

## Redis and realtime

Redis uses separate logical DBs for cache, realtime and Celery. The post-run instance reported
`maxmemory=0`, 2,132,704 bytes used and 2,186,656 bytes peak after restart; Docker observed about
44.3MiB total Redis container memory while 300 sockets were active. Customer operations must set
host memory/alert limits; an application-level eviction cap is intentionally not invented here.
...
| --------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Media           | ~100 GB/year plus temporary upload/headroom                                            | Representative fixture was 8,252 bytes; customer retention horizon x growth + operating headroom remains `PENDING`. |
| PostgreSQL      | 20 000 messages/day peak plus reads/audit/search indexes                               | Post-load database was 93,345,471 bytes at 20,975 messages; customer growth/retention horizon remains `PENDING`.    |
