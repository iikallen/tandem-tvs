...
Production always starts through `compose.yaml` plus `compose.prod.yaml` and an operator-owned secret env file. It uses local Tandem authentication, `PORTAL_ADAPTER=unavailable`, one-shot migrations and exact SHA-tagged images. The old Stage 1 mock/passwordless instructions are no longer valid.
