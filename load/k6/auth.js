...
export function ensureAuthenticated() {
  ...
  const csrf = http.get(`${baseUrl}/api/v1/auth/csrf`, { tags: { kind: 'auth' } });
  ...
  if (!check(login, { 'login succeeded': (response) => response.status === 200 })) {
...
export function jsonParams(kind) {
  return {
    headers: { 'Content-Type': 'application/json', 'X-CSRFToken': ensureAuthenticated() },
    tags: { kind },
  };
}
...
export function firstConversation() {
  ensureAuthenticated();
  const response = http.get(`${baseUrl}/api/v1/messenger/conversations?page_size=50`, {
    tags: { kind: 'inbox' },
  });
  if (!check(response, { 'conversation inbox loaded': (item) => item.status === 200 })) return null;
